...
class NnInferenceClient(BaseInferenceClient):
    ...
    def process_batch(
        ...
        features = [[req.features] for requests in requests_by_symbol.values() for req in requests]

        pinned_preds_buffer = torch.empty((len(features), 4), pin_memory=True) # blocking
        pinned_features = torch.tensor(features, pin_memory=True) # blocking
        ...
        for symbol, symbol_requests in requests_by_symbol.items():
            ...
            for req in symbol_requests:
                x = pinned_features[row].to(device=self.device, non_blocking=True) # H2D
                ...
                pinned_preds_buffer[row].copy_(pred.squeeze(0), non_blocking=True) # D2H
                row += 1
